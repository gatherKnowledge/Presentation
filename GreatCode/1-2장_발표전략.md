...
8진법 컴퓨터도 있었다는 이야기.. 역시 일찍부터 사용됐던 컴퓨터 시스템이라고 한다. 지금 linux에서도 사용되고 있는 파일 권한에 대한 처리를 777로 하는 것 역시 8진 표기법의 흔적이라고 볼 수 있다.
...
---
비즈니스 지향적인 언어란? 프로그램적인 개념은 신경쓰지 않고 로직만으로 개발 가능한 언어. 쉽게 비개발자들도 쓸 수 있게 만들어진 언어,
#### 각 장 넘기는 ment
새로운 목차
수와 수치표기법
시스템 내부에서의 처리
...
수와 수치표기법의 개념이 있다. 처음에 심볼만 있고 자리수에 대한 개념이 없었을 때 잔가지 표기법 같은 것들을 사용했었는데, 불편한 것들이 많았다. --> 왜냐하면 양의 크기와 심볼의 개수가 일대일로 대응되기 때문에 너무 많아진다. --> 10진 위치 표기법을 우선 설명 사람손을 기준으로 만들었기 때문에 사람에게만 편할 뿐 기계에겐 편하지 않다 -->  다른 진법 존재 --> 2,8,16 각 진법을 알아야 하는 이유는 뭐고 어떻다. --> 실제로 컴퓨터 내부에서 사용은 전부 2진법으로 사용되는데, 실제 이진법표기하고 읽는 것은 효율적이지 못하다. 공간낭비의 이야기  --> 16진법을 사용한다.

전 장에서 배운 2진법의 경우 비트의 일대일로 대응이 됨 표현의 한계 --> 비트 스트링, 데이터의 단위 --> 그래서 이렇게만 사용하면 부호 양수만 표현할 수 있다, 음수는 어떻게 표현을 하는가? --> 부호있는 수 개념 없는 수 개념이 있다 --> 실제로 빼기 연산 마이너스는 빼기 연산을 하는것이 아닌 2의 보수 연산을 통해 만들고 더하기 연산을 하는 것이다. -->  실제 연산을 하다보면 모두 똑같은 데이터형을 처리하면 좋지만 그렇지 못하다. 그래서 해야 하는 것이 부호의 확장과 축소이다.

지금까지 정수의 수치표기에 대해서만 설명했는데 소수점에 대한 설명을 시작, 초기엔 고정소수표기를 사용했다. 소프트웨어적으로
--> 2진화 10진표기법에 대한 설명, 대부분의 언어에서는 지원하지 않고 비즈니스 언어인 코볼 같은 곳에서 지원을 해준다.  --> 이러저러한 문제들이 많다, 정확히 표현할 수 없는 것이 가장 큰 문제 그래서 방금 배운BCD코드를 이용한다. --> BCD코드의 단점이 넓은 범위를 표현 할 수 없다는 것인데 두 마리의 토끼를 모두 잡을 수 있는 방법이 있다. --> 스케일 수치 표기법 --> 유리수 표기법
